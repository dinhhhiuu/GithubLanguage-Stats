...
const __filename = fileURLToPath(import.meta.url); // C:\daihoc\SoftWare\github-lang-svg\generate.js
const __dirname = path.dirname(__filename); // C:\daihoc\SoftWare\github-lang-svg

const DATA_DIR = path.join(__dirname, "data"); // C:\daihoc\SoftWare\github-lang-svg\data
if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR);

const COLORS = JSON.parse(
  fs.readFileSync(path.join(__dirname, "const/colors.json"), "utf8")
);
...
async function generate(username) {
  ...
  const barSegments = sorted
    .map(([lang, v]) => {
      ...
      const rect = `<rect x="${xOffset}" y="0" width="${width}" height="20" fill="${COLORS[lang]?.color || "#ccc"}" />`;
      xOffset += width;
      return rect;
    })
    .join("");
  ...
  const leftLabels = left
    .map(([lang, v], i) => `<text x="0" y="${20 + i * 20}" fill="${COLORS[lang]?.color || "#ccc"}">● ${lang}: ${((v / total) * 100).toFixed(1)}%</text>`)
    .join("");

  const rightLabels = right
    .map(([lang, v], i) => `<text x="160" y="${20 + i * 20}" fill="${COLORS[lang]?.color || "#ccc"}">● ${lang}: ${((v / total) * 100).toFixed(1)}%</text>`)
    .join("");
